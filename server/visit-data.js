export async function visitDataResponse(req, env) {
  ...
  const placesDatabaseId = env.NOTION_PLACES_DATABASE_ID
  const visitsDatabaseId = env.NOTION_VISITS_DATABASE_ID

  if (!notionToken) {
    return respond({ error: 'Missing NOTION_TOKEN' }, 500)
  }
  ...
  try {
    ...
    return respond({ error: 'Failed to fetch visit data' }, 500)
  }
}
